...
function init() {
    ...
    <div class='chat-popup'>
		...
		<div class='chat-input-area'>
			<input type='text' autofocus class='chat-input' onkeypress='return givenUserInput(event)' placeholder='సందేశాన్ని టైప్ చేయండి ...' autocomplete='off'>
			<button class='chat-submit'><i class='material-icons'>send</i>
            <button class='chat-mic'><i class='material-icons'>mic</i></button>
            <button class='attach-mic'><i class='material-icons'>send</i></button>
		</div>
    ...
    chatMic = document.querySelector(".chat-mic")
    attachMic = document.querySelector(".attach-mic")
    ...
    attachMic.addEventListener("click", () => {
        let userResponse = chatInput.value.trim();
        if (userResponse !== "") {
            setUserResponse();
            send(userResponse)
...
function chatbotTheme(theme) {
    ...
    if (theme === "orange") {
        ...
        chatMic.style.backgroundColor = orange.color;
        attachMic.style.backgroundColor = orange.color;
        ...
        chatMic.style.backgroundColor = purple.color;
        attachMic.style.backgroundColor = purple.color;
    }
}
